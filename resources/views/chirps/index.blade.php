                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('chirps.store') }}">
                        ...
                        <textarea name="mensaje"
                            ...
                            placeholder="{{ __('What\'s on your mind?') }}">{{ old('mensaje') }}</textarea>
                        <x-input-error :messages="$errors->get('mensaje')" />
                        @error('mensaje')
                            {{ $message }}
                        @enderror
                        <!-- <button>Chirps</button> -->
                    ...
                    <div class="p-6 flex space-x-2">
                            ...
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    @unless ($chirp->created_at->eq($chirp->updated_at))
                                        <small class="text-sm text-gray-600 dark:text-gray-400">&middot; {{ __('edited') }}
                                        </small>
                                    @endunless
                                </div>
                            </div>
                        ...
                        @can('update', $chirp)
                            <x-dropdown>
                                <x-slot name="trigger">
                                    <button>
                                        <svg class="w-5 h-5 text-gray-500 dark:text-gray-300" data-slot="icon"
